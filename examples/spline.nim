...
proc Points*(f: TFunction, start, stop: float64, segments: int): TPoints =
  result = @[]
  for i in 0..segments-1:
    let a = float64(i) * (stop - start) / float64(segments) + start
...
proc Points*(f: Curve, start, stop: float64, segments: int): TPoints =
  result = @[]
  for i in 0..segments-1:
    let a = float64(i) * (stop - start) / float64(segments) + start
...
proc quadraticBezier*(path: var PathStorage, tf: TransformedFunction, start, stop: float64, segments: int) =
  var startPoint = point2d(tf.Input(start), tf.Val(start))
  ...
  for i in 0..segments-1:
    let aa = float64(i) * (stop - start) / float64(segments) + start
    let A = tf.Input(aa)
    let bb  = float64(i + 1) * (stop - start) / float64(segments) + start
    let B = tf.Input(bb)
    let denom = df(bb) - df(aa)
    let P1X = (f(aa) - f(bb) - A * df(aa) + B * df(bb)) / denom
    let P1Y = (f(bb) * df(aa) - (f(aa) + (B - A) * df(aa)) * df(bb)) / -denom
    path.curve3(P1X, P1Y, B, f(bb))

proc cubicBezier*(path: var PathStorage, tf: TransformedFunction, start, stop: float64, segments: int) =
  var startPoint = point2d(tf.Input(start),tf.Val(start))
  path.moveTo(startPoint.x, startPoint.y)
  ...
  for i in 0..segments-1:
    let aOrig = float64(i) * (stop - start) / float64(segments) + start
    let aa = tf.Input(aOrig)
    let bOrig = float64(i + 1) * (stop - start) / float64(segments) + start
    ...
    path.curve4(P1.x, P1.y, P2.x, P2.y, P3.x, P3.y)

proc cubicBezier*(path: var PathStorage, tc: Curve, start, stop: float64, segments: int) =
  path.moveTo(tc.X(0), tc.Y(0))
  for i in 0..segments-1:
    let t0 = float64(i) * (stop - start) / float64(segments) + start
    let t1 = float64(i + 1) * (stop - start) / float64(segments) + start
