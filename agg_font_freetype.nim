...
#[
  ...
  var
    v_last, v_control, v_start: FT_Vector
    x1, y1, x2, y2, x3, y3: float64
    point, limit = ptr FT_Vector
    tags: cstring
    first: int # index of first point in contour
    tag: int  # current point's state

  first = 0
  for n in 0..<outline.n_contours:
    var last = int(outline.contours[n])  # index of last point in contour
    ...
    if tag == FT_CURVE_TAG_CONIC:
      ...
      if FT_CURVE_TAG(outline.tags[last].ord) == FT_CURVE_TAG_ON:
    ...
    if flipY: y1 = -y1
    mtx.transform(x1, y1)
    path.moveTo(ValueT(dbl_to_int26p6(x1)), ValueT(dbl_to_int26p6(y1)))
    ...
    while point < limit:
      ...
      of FT_CURVE_TAG_ON:  # emit a single line_to
        ...
        if flipY: y1 = -y1
        mtx.transform(x1, y1)
        path.lineTo(ValueT(dbl_to_int26p6(x1)), ValueT(dbl_to_int26p6(y1)))
        continue
      of FT_CURVE_TAG_CONIC:  # consume conic arcs
        ...
        while point < limit:
          ...
          if(tag == FT_CURVE_TAG_ON)
          {
              ...
              mtx.transform(&x1, &y1)
              mtx.transform(&x2, &y2)
              path.curve3(ValueT(dbl_to_int26p6(x1)),
                          ValueT(dbl_to_int26p6(y1)),
                          ValueT(dbl_to_int26p6(x2)),
                          ValueT(dbl_to_int26p6(y2)))
              continue;
      ...
      default:  # FT_CURVE_TAG_CUBIC
      {
          ...
          if(point <= limit)
          {
              ...
              mtx.transform(&x2, &y2)
              mtx.transform(&x3, &y3)
              path.curve4(ValueT(dbl_to_int26p6(x1)),
                          ValueT(dbl_to_int26p6(y1)),
                          ValueT(dbl_to_int26p6(x2)),
                          ValueT(dbl_to_int26p6(y2)),
                          ValueT(dbl_to_int26p6(x3)),
...
proc decompose_ft_bitmap_gray8[Rasterizer, Scanline, ScanlineStorage](bitmap: var FT_Bitmap,
  ...
  for i in 0.. <int(bitmap.rows):
    ...
    var p = buf
    for j in 0.. <int(bitmap.width):
      if p[] != 0: sl.addCell(x + j, uint(ras.applyGamma(int(p[]))))
      inc p
  ...
  if self.mLibraryInitialized:
    ...
    if self.mLastError == 0:
      ...
      of glyph_ren_agg_mono:
        if FT_IS_SCALABLE(self.mCurFace): self.mGlyphRendering = glyph_ren_agg_mono
        else: self.mGlyphRendering = glyph_ren_native_mono
      of glyph_ren_agg_gray8:
        if FT_IS_SCALABLE(self.mCurFace): self.mGlyphRendering = glyph_ren_agg_gray8
        else: self.mGlyphRendering = glyph_ren_native_gray8
