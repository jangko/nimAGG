...
type
   ConvCurve*[VertexSource, C3, C4] = object
     mSource: ptr VertexSource
     mLastX, mLastY: float64
     mCurve3: C3
     mCurve4: C4

proc initConvCurve1*[VS, C3, C4](source: var VS): ConvCurve[VS, C3, C4] =
  result.mSource = source.addr
  result.mLastX = 0.0
  result.mLastY = 0.0
  result.mCurve3 = construct(C3)
  result.mCurve4 = construct(C4)

proc initConvCurve*[VS](source: var VS): ConvCurve[VS, Curve3, Curve4] =
  result = initConvCurve1[VS, Curve3, Curve4](source)

proc attach*[VS, C3, C4](self: var ConvCurve[VS, C3, C4], source: var VS) =
  self.mSource = source.addr

proc approximationMethod*[VS, C3, C4](self: var ConvCurve[VS, C3, C4], v: CurveApproximationMethod) =
  self.mCurve3.approximationMethod(v)
  self.mCurve4.approximationMethod(v)

proc approximationMethod*[VS, C3, C4](self: ConvCurve[VS, C3, C4], ): CurveApproximationMethod =
  self.mCurve4.approximationMethod()

proc approximationScale*[VS, C3, C4](self: var ConvCurve[VS, C3, C4], s: float64) =
  self.mCurve3.approximationScale(s)
  self.mCurve4.approximationScale(s)

proc approximationScale*[VS, C3, C4](self: ConvCurve[VS, C3, C4]): float64 =
  self.mCurve4.approximationScale()

proc angleTolerance*[VS, C3, C4](self: var ConvCurve[VS, C3, C4], v: float64) =
  self.mCurve3.angleTolerance(v)
  self.mCurve4.angleTolerance(v)

proc angleTolerance*[VS, C3, C4](self: ConvCurve[VS, C3, C4]): float64 =
  self.mCurve4.angleTolerance()

proc cuspLimit*[VS, C3, C4](self: var ConvCurve[VS, C3, C4], v: float64) =
  self.mCurve3.cuspLimit(v)
  self.mCurve4.cuspLimit(v)

proc cuspLimit*[VS, C3, C4](self: ConvCurve[VS, C3, C4]): float64 =
  self.mCurve4.cuspLimit()

proc rewind*[VS, C3, C4](self: var ConvCurve[VS, C3, C4], pathId: int) =
...
proc vertex*[VS, C3, C4](self: var ConvCurve[VS, C3, C4], x, y: var float64): uint =
