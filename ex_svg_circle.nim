...
import agg_renderer_base, agg_conv_curve
import agg_renderer_scanline, agg_color_rgba, agg_pixfmt_rgb
import agg_path_storage, agg_trans_affine, agg_conv_transform, agg_conv_stroke
import agg_bounding_rect, agg_bezier_arc, colors, agg_platform_support
...
proc newApp(format: PixFormat, flipY: bool): App =
  ...
  result.helmet = createHelmet(frameWidth, frameHeight, 1.5)
...
method onDraw(app: App) =
  var
    ...
    curve  = initConvCurve(app.helmet.path)
    trans  = initConvTransform(curve, app.helmet.mtx)

  rb.clear(initRgba(1,1,1))
  renderAllPaths(ras, sl, ren, trans, app.helmet.colors, app.helmet.pathIdx, app.helmet.numPaths)

proc main(): int =
  var app = newApp(pix_format_bgr24, flipY)
  app.caption("AGG Example. SVG Circle")

  if app.init(frameWidth, frameHeight, {}, "svg_circle"):
    return app.run()
