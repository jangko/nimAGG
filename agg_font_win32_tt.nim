import agg_basics, agg_font_types, winapi, agg_scanline_storage_aa
import agg_scanline_storage_bin, agg_scanline_u, agg_scanline_bin
import agg_path_storage_integer, agg_rasterizer_scanline_aa, agg_conv_curve
import agg_trans_affine, agg_bitset_iterator, strutils, algorithm
import math, agg_renderer_scanline
...
proc prepareGlyph*(self: FontEngineWin32TTBase, glyphCode: int): bool =
  ...
  if self.mDC != NULL and self.mCurFont != NULL:
    ...
    elif self.mGlyphRendering in {glyph_ren_outline, glyph_ren_agg_mono, glyph_ren_agg_gray8}:
      format = GGO_NATIVE
...
proc loadKerningPairs*(self: FontEngineWin32TTBase) =
  if self.mDC != NULL and self.mCurFont != NULL:
    if self.mKerningPairs == nil:
      self.mKerningPairs = newSeq[KERNINGPAIR](16384-16)
