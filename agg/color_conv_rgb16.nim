...
template color_conv_rgbAAA_rgb24(R, B: int, x: untyped) =
  proc x*(dst, src: ptr uint8, width: int) {.procvar.} =
    ...
    doWhile w != 0:
      ...
      d[R] = cast[uint8](rgb shr 22)
      d[1] = cast[uint8](rgb shr 12)
      d[B] = cast[uint8](rgb shr 2)
      inc(s, 4)
      inc(d, 3)
      dec w
...
template color_conv_rgbBBA_rgb24(R, B: int, x: untyped) =
  proc x*(dst, src: ptr uint8, width: int) {.procvar.} =
    ...
    doWhile w != 0:
      let rgb = cast[ptr uint32](s)[]
      d[R] = cast[uint8](rgb shr 24)
      d[1] = cast[uint8](rgb shr 13)
      d[B] = cast[uint8](rgb shr 2)
      inc(s, 4)
      inc(d, 3)
      dec w
...
template color_conv_bgrABB_rgb24(B, R: int, x: untyped) =
  proc x*(dst, src: ptr uint8, width: int) {.procvar.} =
    ...
    doWhile w != 0:
      ...
      d[R] = cast[uint8](bgr shr 3)
      d[1] = cast[uint8](bgr shr 14)
      d[B] = cast[uint8](bgr shr 24)
      inc(s, 4)
      inc(d, 3)
      dec w
