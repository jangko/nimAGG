...
#[
bool on_mouse_button_down(self: var MeshCtrl, double x, double y, unsigned flags)
{
    if(flags & 1)
    {
        unsigned i;
        ...
        {
            if(calc_distance(x, y, self.vertices[i].x, self.vertices[i].y) < 5)
            {
                m_drag_idx = i;
                m_drag_dx = x - self.vertices[i].x;
                m_drag_dy = y - self.vertices[i].y;
                return true;
            }
        }
    }
...
bool on_mouse_move(self: var MeshCtrl, double x, double y, unsigned flags)
{
    if(flags & 1)
    {
        if(m_drag_idx >= 0)
        {
            self.vertices[m_drag_idx].x = x - m_drag_dx;
            self.vertices[m_drag_idx].y = y - m_drag_dy;
            return true;
        }
    }
    else
...
bool on_mouse_button_up(self: var MeshCtrl, double x, double y, unsigned flags)
{
...
proc initApp(): App =
  ...
  result.mesh.generate(20, 20, 17, 17, 40, 40)
...
proc onDraw() =
  ...
  let tm = (cpuTime() - startTime) * 1000.0
  ...
  saveBMP24("gouraud_mesh.bmp", buffer, frameWidth, frameHeight)
