...
type
  FT_Outline_MoveToFunc* = proc (to: ptr FT_Vector; user: pointer): cint {.ftcallback.}
  FT_Outline_LineToFunc* = proc (to: ptr FT_Vector; user: pointer): cint {.ftcallback.}
  FT_Outline_ConicToFunc* = proc (control: ptr FT_Vector; to: ptr FT_Vector;
                               user: pointer): cint {.ftcallback.}
  FT_Outline_CubicToFunc* = proc (control1: ptr FT_Vector; control2: ptr FT_Vector;
                               to: ptr FT_Vector; user: pointer): cint {.ftcallback.}
...
type
  ...
  FT_SpanFunc* = proc (y: cint; count: cint; spans: ptr FT_Span; user: pointer) {.ftcallback.}
  FT_Raster_Span_Func* = FT_SpanFunc
  FT_Raster_BitTest_Func* = proc (y: cint; x: cint; user: pointer): cint {.ftcallback.}
  FT_Raster_BitSet_Func* = proc (y: cint; x: cint; user: pointer) {.ftcallback.}
...
type
  ...
  FT_Raster_NewFunc* = proc (memory: pointer; raster: ptr FT_Raster): cint {.ftcallback.}
  FT_Raster_DoneFunc* = proc (raster: FT_Raster) {.ftcallback.}
  FT_Raster_ResetFunc* = proc (raster: FT_Raster; pool_base: ptr cuchar; pool_size: culong) {.ftcallback.}
  FT_Raster_SetModeFunc* = proc (raster: FT_Raster; mode: culong; args: pointer): cint {.ftcallback.}
  FT_Raster_RenderFunc* = proc (raster: FT_Raster; params: ptr FT_Raster_Params): cint {.ftcallback.}
